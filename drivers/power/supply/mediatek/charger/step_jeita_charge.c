...
static void monitor_typec_burn(struct charger_manager *info)
{
	...
	if (!gpio_is_valid(info->vbus_control_gpio) && product_name == PISSARROPRO)
		return;
	...
	if ((product_name == PISSARROPRO || product_name == PISSARRO) && type_temp <= TYPEC_BURN_TEMP && type_temp > TYPEC_BURN_REPORT_TEMP && info->typec_burn_flag != 1) {
		power_supply_changed(info->usb_psy);
		info->typec_burn_flag = 1;
          	chr_info("typec burn temp = %d\n",type_temp);
	} else if ((product_name == PISSARROPRO || product_name == PISSARO) && type_temp > TYPEC_BURN_TEMP && info->typec_burn_flag != 2) {
		...
		info->typec_burn_flag = 2;
          	chr_info("typec burn temp = %d\n",type_temp);
	} else if ((product_name == PISSARROPRO || product_name == PISSARRO) && type_temp < TYPEC_BURN_LOW_TEMP && (info->typec_burn || info->typec_burn_flag == 1) && info->typec_burn_flag != 3) {
	...
	if (product_name == PISSARROPRO && type_temp >= TYPEC_BURN_TEMP && info->typec_burn_flag != 4) {
		if (!info->typec_burn_wakelock->active)
			__pm_stay_awake(info->typec_burn_wakelock);
		info->typec_burn = true;
		while (retry_count) {
		...
		info->typec_burn_flag = 4;
          	chr_info("typec burn temp = %d\n",type_temp);
	} else if (info->typec_burn && product_name == PISSARROPRO && type_temp <= TYPEC_BURN_TEMP - TYPEC_BURN_HYST && info->typec_burn_flag != 5) {
		info->typec_burn = false;
...
static void handle_jeita_charge(struct charger_manager *info)
{
	...
	if (jeita_vbat_low) {
		if (info->vbat < (info->jeita_fcc_cfg[info->jeita_chg_index[0]].extra_threshold + (product_name == PISSARRO ? 50 : 100))) {
	...
	if (is_between(info->jeita_fcc_cfg[0].low_threshold, info->jeita_fcc_cfg[info->step_jeita_tuple_count - 1].high_threshold, info->tbat) && !info->typec_burn
		 && !info->charge_full) {
		out_jeita = false;
		if((product_name == PISSARRO || product_name == PISSARRO) && info->jeita_fv_cfg[info->jeita_chg_index[0]].value == 4100 && info->vbat > 4100){
			vote(info->bbc_en_votable, BBC_ENABLE_VOTER, true, 0);
		} else {
			vote(info->bbc_en_votable, BBC_ENABLE_VOTER, true, 1);
		}
	} else {
		if (product_name == PISSARRO && !out_jeita && info->vbus >= 6000) {
	...
	vote(info->bbc_fv_votable, JEITA_CHARGE_VOTER, true, info->jeita_fv_cfg[info->jeita_chg_index[0]].value - (product_name != PISSARROPRO ? info->diff_fv_val : 2 * info->diff_fv_val));
...
static int handle_ffc_charge(struct charger_manager *info)
{
	...
	if (!info->charge_full &&!info->recharge && info->entry_soc <= info->ffc_high_soc && (product_name == PISSARROPRO ? is_between(MIN_FFC_JEITA_CHG_INDEX, MAX_FFC_JEITA_CHG_INDEX, info->jeita_chg_index[0]) : info->jeita_chg_index[0] == MIN_FFC_JEITA_CHG_INDEX) &&
	...
	if (product_name == PISSARROPRO && gpio_is_valid(info->vbus_control_gpio) && info->vbus > 3600 && info->soc > info->ffc_high_soc)
		info->ffc_enable = false;
	...
	info->fv_effective = (info->ffc_enable ? info->fv_effective : info->fv) - (product_name != PISSARROPRO ? info->diff_fv_val : 2 * info->diff_fv_val);
...
static void check_full_recharge(struct charger_manager *info)
{
	...
	int bbc_full = (product_name == PISSARROPRO) ? MP2762_CHARGE_STATUS_DONE : MT6360_CHARGE_STATUS_DONE;
	int iterm = 0;
	int ret = 0;
	union power_supply_propval pval = {0,};
	...
	if (info->recheck_count < 2 && product_name == PISSARROPRO) {
		return;
	}
	...
	if (info->charge_full) {
		...
		if(product_name == PISSARROPRO) {
			...
			if (info->vbat <= info->fv_effective - (product_name == PISSARRO ? 180 : 200))
			      recharge_count++;
			else
			      recharge_count = 0;
		}
		...
		if(info->dynamic_fv_hold == 1){
			//Nothing to do
		}else if(product_name == PISSARROPRO && (info->vbat > info->fv_effective - info->fv_ffc_delta)){
			info->dynamic_fv_down_cnt++;
		}else if(product_name != PISSARROPRO && info->ffc_enable && (info->vbat > info->fv_effective - info->fv_ffc_delta)){
			info->dynamic_fv_down_cnt++;
		}else{
		...
		if(info->dynamic_fv_down_cnt >= 5) {
			...
			if(fv_result + (product_name == PISSARROPRO ? 100 : 40) > info->fv_effective){
				vote(info->bbc_fv_votable, DYNAMIC_FV_VOTER, true, fv_result - (product_name == PISSARROPRO ? 25 : 10));
		...
		if (((product_name == PISSARROPRO) && (info->soc == 100) && (-info->ibat <= (info->iterm_effective + threshold_ma)) && (info->vbat >= (info->fv_effective - threshold_mv))) ||
			...
			(product_name == PISSARRO && info->ffc_enable && (info->vbat >= (4471 - info->diff_fv_val)) && info->fg_full && (-info->ibat <= (iterm + threshold_ma))) ||
			(product_name == PISSARROPRO && info->ffc_enable && info->vbat >= 8930 && (-info->ibat <= (iterm + threshold_ma)))) {
			if (info->soc == 100) {
				full_count++;
				full_wa_count = 0;
		...
		if (full_count >= 4 || (product_name == PISSARROPRO && info->cp_taper)) {
...
static void monitor_thermal_limit(struct charger_manager *info)
{
	...
	if (product_name == PISSARRO || product_name == PISSARROPRO) {
...
static void quick_update_battery_info(struct charger_manager *info)
{
	...
	if((product_name == PISSARROPRO) && (info->cycle_count_status == CYCLE_COUNT_FRESH)){
